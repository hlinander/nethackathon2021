...
# of the relation between Nix and the Nix Flakes.
{
  ...
  inputs = {
    ...
    helix-pkg.url = "github:helix-editor/helix";
  };
  ...
  # ```
  outputs =
    { self, flake-parts, helix-pkg, ... }@inputs:
    let
...

      {
        ...
        perSystem =
          ...
          in
          {
            ...
            legacyPackages = {
              llamaPackages = pkgs.callPackage .devops/nix/scope.nix { inherit llamaVersion helixmaster; };
              llamaPackagesWindows = pkgs.pkgsCross.mingwW64.callPackage .devops/nix/scope.nix { inherit llamaVersion; };
              llamaPackagesCuda = pkgsCuda.callPackage .devops/nix/scope.nix { inherit llamaVersion helixmaster; };
